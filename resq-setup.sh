...
sudo cp dxc-masternode-setup/dxc-v4.4.1/dxc* /usr/bin/
sudo chmod 755 -R ~/dxc-masternode-setup
sudo chmod 755 /usr/bin/dxc*

# Deploy masternode monitoring script
cp ~/dxc-masternode-setup/nodemon.sh /usr/local/bin
sudo chmod 711 /usr/local/bin/nodemon.sh

#Create dxc datadir
if [ ! -f ~/.dixicore/dixi.conf ]; then 
	sudo mkdir ~/.dixicore
fi

echo -e "${YELLOW}Creating dixi.conf...${NC}"

# If genkey was not supplied in command line, we will generate private key on the fly
if [ -z $genkey ]; then
    cat <<EOF > ~/.dixicore/dixi.conf
rpcuser=dixirpc
rpcpassword=$rpcpassword
EOF

    sudo chmod 755 -R ~/.dixicore/dixicoin.conf
    ...
    #Stopping daemon to create dixi.conf
    stop_daemon
    delay 30
fi
...
${YELLOW}cat ~/.dixicore/dixi.conf${NC}

Here is your dixi.conf generated by this script:
-------------------------------------------------${YELLOW}"
cat ~/.dixicore/dixi.conf
echo -e "${NC}-------------------------------------------------

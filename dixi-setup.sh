#!/bin/bash
# dxc Masternode Setup Script V1.3 for Ubuntu 16.04 LTS
# Credit goes to Allroad [FasterPool.com] (c) 2018 
# Script will attempt to autodetect primary public IP address
# and generate masternode private key unless specified in command line
#
# Usage:
# bash dxc-setup.sh [Masternode_Private_Key]
#
#
...
clear
echo -e "${YELLOW}dixicoin Masternode Setup Script V1.0 for Ubuntu 16.04 LTS${NC}"
...
#Installing Daemon
cd ~
#sudo rm Dixicoin-4.4.1-x86_64-pc-linux-gnu.zip
#wget https://github.com/Dixicoin-DXC/Dixicoin/releases/download/v4.4.1/Dixicoin-4.4.1-x86_64-pc-linux-gnu.zip
#sudo unzip Dixicoin-4.4.1-x86_64-pc-linux-gnu.zip --strip-components 1 --directory /usr/bin
#sudo rm Dixicoin-4.4.1-x86_64-pc-linux-gnu.zip

stop_daemon

# Deploy binaries to /usr/bin
sudo cp ~/dixi-masternode-setup/dxc-v4*/dix* /usr/bin/
sudo chmod 755 -R ~/dixi-masternode-setup/
sudo chmod 755 /usr/bin/dixicoin*

# Deploy masternode monitoring script
sudo cp ~/dixi-masternode-setup/nodemon.sh /usr/local/bin
sudo chmod 711 /usr/local/bin/nodemon.sh

#Create dxc datadir
if [ ! -f ~/.dixicoin/dixicoin.conf ]; then 
	sudo mkdir ~/.dixicoin
fi

echo -e "${YELLOW}Creating dixicoin.conf...${NC}"

# If genkey was not supplied in command line, we will generate private key on the fly
if [ -z $genkey ]; then
    cat <<EOF > ~/.dixicoin/dixicoin.conf
rpcuser=dixirpc
...
EOF
    ...
    genkey=$(dixicoin-cli masternode genkey)
    if [ -z "$genkey" ]; then
    ...
    #Stopping daemon to create dixicoin.conf
    stop_daemon
    delay 30
fi

# Create dixicoin.conf
cat <<EOF > ~/.dixicoin/dixicoin.conf
rpcuser=dixirpc
rpcpassword=$rpcpassword
...
#Finally, starting dixi daemon with new dixicoin.conf
dixicoind
delay 5
...
for your Hot Wallet (the wallet with your dixicoin collateral funds):
======================================================================== \a"
...
To introduce your new masternode to the dixicoin network, you need to
...
Currently your masternode is syncing with the dixicoin network...
...
${YELLOW}cat ~/.dixicoin/dixicoin.conf${NC}

Here is your dixicoin.conf generated by this script:
-------------------------------------------------${YELLOW}"
cat ~/.dixicoin/dixicoin.conf
echo -e "${NC}-------------------------------------------------

NOTE: To edit dixicoin.conf, first stop the dixid daemon,
then edit the dixicoin.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the dixicoind daemon back up:

to stop:   ${YELLOW}eden-cli stop${NC}
to edit:   ${YELLOW}nano ~/.dixicoin/dixicoin.conf${NC}
...
Enjoy your dixi Masternode and thanks for using this setup script!

If you found it helpful, please donate dixi to:
